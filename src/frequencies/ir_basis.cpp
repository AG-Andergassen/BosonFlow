...
#include <frequencies/conventions.h>

IR_basis_factory_t::IR_basis_factory_t(double beta, double error)
{
    m_beta = beta;
    m_error = error;
}

IR_2p_basis_data_t IR_basis_factory_t::make_2p_basis(int total_number_of_freqs, double bandwidth, Statistics zeta, bool augment_with_constant)
{
    initialise_julia_context_and_headers();

    jl_eval_string((std::string("beta = ") + std::to_string(m_beta)).c_str());
    jl_eval_string((std::string("omega_max = ") + std::to_string(bandwidth)).c_str());
    jl_eval_string((std::string("epsilon = ") + std::to_string(m_error)).c_str());
	...
	julia_basis_string = std::string("SparseIR.MatsubaraSampling(AugmentedBasis(") + julia_basis_string + ", MatsubaraConst))";

    jl_eval_string((std::string("matsubara_sampling = SparseIR.MatsubaraSampling(")+julia_basis_string+")").c_str());
    jl_eval_string((std::string("everywhere_sampling = SparseIR.MatsubaraSampling(")+julia_basis_string+", sampling_points="+make_julia_2p_freq_array(total_number_of_freqs)+")").c_str());
    get_julia_2Darray_complex("everywhere_sampling.matrix * pinv(matsubara_sampling.matrix)", basis_data.E);


    if (zeta == Statistics::FERMI)
	get_julia_array( "(Int.(matsubara_sampling.sampling_points) .- 1) ./ 2 ", basis_data.sampling_freqs_full);
    else
	get_julia_array( "Int.(matsubara_sampling.sampling_points) ./ 2 ", basis_data.sampling_freqs_full);
...
IR_3p_basis_data_t IR_basis_factory_t::make_3p_basis(int total_number_of_freqs_bos, double bandwidth_bos, int total_number_of_freqs_ferm, double bandwidth_ferm, bool augment_with_constant)
{
    initialise_julia_context_and_headers();

    jl_eval_string((std::string("beta = ") + std::to_string(m_beta)).c_str());
    jl_eval_string((std::string("omega_max_b = ") + std::to_string(bandwidth_bos)).c_str());
    jl_eval_string((std::string("omega_max_f = ") + std::to_string(bandwidth_ferm)).c_str());
    jl_eval_string((std::string("epsilon = ") + std::to_string(m_error)).c_str());

    IR_3p_basis_data_t basis_data;
    ...
    get_julia_tuple3array<int>( "map(w -> Int.(Int(w[1])/2, (Int(w[2])-1)/2, (Int(w[3])-1)/2 ), eval3_sampling.wsample)", basis_data.sampling_freqs_full);
    
    {
	std::string pp_interpolating_freqs = make_julia_3p_freq_array([](int W, int w){ 
		int w2, w4;
	...
	std::string ph_interpolating_freqs = make_julia_3p_freq_array([](int W, int w){ 
		int w2, w3;
		Symmetrised3P::ph_to_full_freqs(W, w, w2, w3);
		return std::make_tuple(2*W, 2*w2+1, 2*w3+1);
	    }, total_number_of_freqs_bos, total_number_of_freqs_ferm);
	std::string xph_interpolating_freqs = make_julia_3p_freq_array([](int W, int w){ 
		int w1, w2;
		Symmetrised3P::xph_to_full_freqs(W, w, w1, w2);
		return std::make_tuple(2*W, 2*w1+1, 2*w2+1);
	    }, total_number_of_freqs_bos, total_number_of_freqs_ferm);
	
	jl_eval_string((std::string("eval3_pp = OvercompleteIR.MatsubaraEval(basis3, sampling_points=")+pp_interpolating_freqs+")").c_str());
	jl_eval_string((std::string("eval3_ph = OvercompleteIR.MatsubaraEval(basis3, sampling_points=")+ph_interpolating_freqs+")").c_str());
	jl_eval_string((std::string("eval3_xph = OvercompleteIR.MatsubaraEval(basis3, sampling_points=")+xph_interpolating_freqs+")").c_str());
...
IR_4p_basis_data_t IR_basis_factory_t::make_4p_basis(int total_number_of_freqs_bos, double bandwidth_bos, int total_number_of_freqs_ferm, double bandwidth_ferm, bool augment_with_constant)
{
    initialise_julia_context_and_headers();
    ...
    get_julia_tuple4array<int>( "map(w -> (Int.(w).- 1)./2, eval4_sampling.wsample)", basis_data.sampling_freqs_full);
	...
	jl_eval_string((std::string("eval4_pp = OvercompleteIR.MatsubaraEval(basis4, sampling_points=")+pp_interpolating_freqs+")").c_str());
	jl_eval_string((std::string("eval4_ph = OvercompleteIR.MatsubaraEval(basis4, sampling_points=")+ph_interpolating_freqs+")").c_str());
	jl_eval_string((std::string("eval4_xph = OvercompleteIR.MatsubaraEval(basis4, sampling_points=")+xph_interpolating_freqs+")").c_str());
...
IR_2p_basis_data_t IR_basis_factory_t::make_2p_basis_with_max_sampling_frequency(int max_sampling_frequency, Statistics zeta, bool augment_with_constant)
{
...
IR_3p_basis_data_t IR_basis_factory_t::make_3p_basis_with_max_sampling_frequency(int max_sampling_frequency_bos, int max_sampling_frequency_ferm, bool augment_with_constant)
{
    double bandwidth_ferm = obtain_bandwidth_from_max_sampling_frequency(max_sampling_frequency_ferm, Statistics::FERMI);
    double bandwidth_bos = obtain_bandwidth_from_max_sampling_frequency(max_sampling_frequency_bos, Statistics::BOSE);
...
IR_4p_basis_data_t IR_basis_factory_t::make_4p_basis_with_max_sampling_frequency(int max_sampling_frequency_bos, int max_sampling_frequency_ferm, bool augment_with_constant)
{
    double bandwidth_ferm = obtain_bandwidth_from_max_sampling_frequency(max_sampling_frequency_ferm, Statistics::FERMI);
    double bandwidth_bos = obtain_bandwidth_from_max_sampling_frequency(max_sampling_frequency_bos, Statistics::BOSE);
...
double IR_basis_factory_t::obtain_bandwidth_from_max_sampling_frequency(int max_sampling_frequency, Statistics zeta)
{
    initialise_julia_context_and_headers();
    
    jl_eval_string((std::string("beta = ").c_str() + std::to_string(m_beta)).c_str());
    jl_eval_string((std::string("epsilon = ").c_str() + std::to_string(m_error)).c_str());
    
    // note: function is monotonically increasing (step wise)
    auto delta_max_sampling_frequency = [max_sampling_frequency, zeta, this](double bandwidth){
	jl_eval_string((std::string("omega_max = ") + std::to_string(bandwidth)).c_str());
	jl_eval_string("basis_f, basis_b = SparseIR.finite_temp_bases(beta, omega_max, epsilon)");
...
void IR_basis_factory_t::initialise_julia_context_and_headers()
{
    jl_init();
...
void IR_basis_factory_t::destroy_julia_context()
{
    jl_atexit_hook(0);
}

std::string IR_basis_factory_t::make_julia_2p_freq_array(std::vector<int> freq_indices)
{
...
std::string IR_basis_factory_t::make_julia_2p_freq_array(unsigned total_number_of_freqs)
{
    std::string str;
...
std::string IR_basis_factory_t::make_julia_3p_freq_array(std::function<std::tuple<int, int, int>(int, int) > conv_freq_to_full_freq, unsigned total_number_of_freqs1, unsigned total_number_of_freqs2)
{
    std::string str = "[";
    ...
    str += make_3_freq(conv_freq_to_full_freq(first_freq1, first_freq2) );
    for (int omega = first_freq1 + 2; omega <= final_freq1; omega += 2)
	for (int nu = first_freq2 + 2; nu <= final_freq2; nu += 2)
	    str += "," + make_3_freq(conv_freq_to_full_freq(omega, nu));
...
std::string IR_basis_factory_t::make_julia_4p_freq_array(std::function<std::tuple<int, int, int, int>(int, int, int) > conv_freq_to_full_freq, unsigned total_number_of_freqs1, unsigned total_number_of_freqs2, unsigned total_number_of_freqs3)
{
    std::string str = "[";
