...
class AndersonImpurity : public Hubbard<0>
{
 public:
    AndersonImpurity() = delete;
    
    static const std::string GetName(){return "Anderson Impurity";};
    ...
    {
        return {{ "U", AndersonImpurityParams::UINT }, {"Mu", AndersonImpurityParams::MU}, {"KDIM", K_DIM}, {"FineMoms", GetFineMomentaCount()}, {"RefdMoms", GetRefinedMomentaCount()}, {"FFShellCount", FORMFACTOR_SHELL_COUNT}, {"BATH_LEVEL_COUNT", AndersonImpurityParams::BATH_LEVEL_COUNT}};
    };


    static std::complex<double> E(const int idx_k, const int idx_w);

    static double E_of_p_coord(const coord_t<0> p_coord);
