...
class SquareHubbard : public Hubbard<2>
{
 public:
    SquareHubbard() = delete;
    
    static const std::string GetName(){return "Square Hubbard";};
    ...
    {
        return {{ "U", SquareHubbardParams::UINT }, { "V", SquareHubbardParams::VINT }, { "TP", SquareHubbardParams::T_PRIME}, {"Mu", SquareHubbardParams::MU}, {"KDIM", K_DIM}, {"FineMoms", GetFineMomentaCount()}, {"RefdMoms", GetRefinedMomentaCount()}, {"FFShellCount", FORMFACTOR_SHELL_COUNT}};
    };


    static double E(const int idx_k, const int idx_w);

    static double E_of_p_coord(coord_t<2> p_coord);
