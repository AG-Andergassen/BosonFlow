...
// face-centered cube hubbard model
class FCCHubbard : public Hubbard<3>
{
 public:
    FCCHubbard() = delete;
    
    static const std::string GetName(){return "FCC Hubbard";};
    ...
    {
        return {{ "U", FCCHubbardParams::UINT }, { "TP", FCCHubbardParams::T_PRIME}, { "TPP", FCCHubbardParams::T_PRIME_PRIME}, {"Mu", FCCHubbardParams::MU}, {"KDIM", K_DIM}, {"FineMoms", GetFineMomentaCount()}, {"RefdMoms", GetRefinedMomentaCount()}, {"FFShellCount", FORMFACTOR_SHELL_COUNT}};
    };

    static double E(const int idx_k, const int idx_w);

    static double E_of_p_coord(coord_t<3> p_coord);
